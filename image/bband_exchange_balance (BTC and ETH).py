...
data = pd.read_csv(r"image/cleaned_data/exchange_balance.csv")
data = data.dropna()
...
for i in range(len(z_threshes)):
    for j in range(len(no_of_days)):
        ...
        df["price"] = test_set["price"].shift(-delay)
        ...
        if SR > best_SR:
            ...
            print("*************************************************************************************************************")
            df.to_csv('image/result/bt_exchange_balance_' + coin + '_' + exchange + '_delay_' + str(delay) + '_test_no_' + str(test) + '.csv', index=False)
        else:
            print(SR)
...
heatmap.set_ylabel('z_threshes', fontsize=12)
heatmap.set_xlabel('no_of_days', fontsize=12)

plt.savefig('image/result/bt_exchange_balance_' + coin + '_' + exchange + '_delay_' + str(delay) + '_test_no_' + str(test) + '.png')
plt.show()
